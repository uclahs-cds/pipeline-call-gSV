...
include { run_sha512sum_Manta; run_sha512sum_Delly; run_sha512sum_Delly as run_sha512sum_regenotype_gSV_Delly; run_sha512sum_Delly as run_sha512sum_regenotype_gCNV_Delly } from './modules/sha512'
...
workflow {
    run_validate_PipeVal(validation_channel)
    ...
    if (params.run_discovery) {
        ...
        if (params.run_delly) {
            ...
            if (params.variant_type.contains(params.GCNV)) {
                ...
                run_sha512sum_Delly(call_gSV_Delly.out.bcf_sv_file.mix(convert_gSV_BCF2VCF_BCFtools.out.vcf_file)) 
                }
    ...
    if (params.run_regenotyping) {
        if (params.variant_type.contains(params.GSV)) {
            ...
            run_sha512sum_regenotype_gSV_Delly(regenotype_gSV_Delly.out.regenotyped_sv_bcf.mix(regenotype_gSV_Delly.out.regenotyped_sv_bcf_csi))
        }
        ...
        if (params.variant_type.contains(params.GCNV)) {
            ...
            run_sha512sum_regenotype_gCNV_Delly(regenotype_gCNV_Delly.out.regenotyped_cnv_bcf.mix(regenotype_gCNV_Delly.out.regenotyped_cnv_bcf_csi))
        }
    }
}
